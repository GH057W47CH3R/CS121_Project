...
UIManager::UIManager(Role role, AppState* app, std::ostream& out, std::istream& in)
    : role_(role), app_(app), out_(out), in_(in) {
}

// Simple array to store contacts
        ...
        // ---------- SELECT function -------------
            ...
            for (int j = 0; j < count; j++) {
               std::string contactValue;
                ...
                if (field == "name") contactValue = contacts[j].getName();
                else if (field == "address") contactValue = contacts[j].getAddress();
                else if (field == "phone") contactValue = contacts[j].getPhone();
                else {
                ...
                for (char c : contactValue) {
                    if (c >= 'A' && c <= 'Z') contactLower += c + ('a' - 'A');
                    else contactLower += c;
                }
                for (char c : value) {
                    if (c >= 'A' && c <= 'Z') valueLower += c + ('a' - 'A');
                    else valueLower += c;
                }
...
Predicate UIManager::parse_predicate() {
    return Predicate{};
}
