...
// We use templates here to avoid having to implement two structs manually
// for const ptrs vs ptrs
...
public:
  ...
  // This constructs an array with T* pointers and allocates size, but does not
  // initialize them
  explicit PtrArray(std::uint32_t capacity)
      : ptrs_(capacity ? new T *[capacity] : nullptr), size_(0),
        capacity_(capacity) {}
  // Destructor releases the pointer array but DOES NOT delete the objects being
  // pointed to
  ...
  void resize(std::uint32_t new_capacity) {
    ...
    if (new_capacity < size_) {
      throw std::runtime_error(
          "attempt to resize PtrArray smaller than current size");
    }
  ...
  T *operator[](std::size_t index) const {
    if (index >= size_)
      throw std::out_of_range("PtrArray index out of range");
    return ptrs_[index];
  }
};
